...
// --- MINI LOGÓ A SWITCHERHEZ (Csak a tiszta ikon) ---
const TurkizIcon = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 190.58 160" xmlns="http://www.w3.org/2000/svg" className={className}>
...
const TOTAL_SETS = 10;
const allVips = Array(TOTAL_SETS).fill(VIP_IMAGES).flat();

export default function Demo3() {
  const [activeSite, setActiveSite] = useState<"turkiz" | "rose">("turkiz"); // STATE A SWITCHERHEZ
  ...
  // --- AUTOMATA GÖRGETÉS LOGIKA CSAK A TÜRKIZ OLDALON ---
  ...
  useEffect(() => {
    if (activeSite === "turkiz" && vipScrollRef.current) {
      const itemWidth = getScrollConfig();
      vipScrollRef.current.style.scrollBehavior = 'auto';
      vipScrollRef.current.scrollLeft = VIP_COUNT * 4 * itemWidth;
      setTimeout(() => {
  ...
  // --- PRELOADER (Csak az első betöltésnél) ---
  useEffect(() => {
  ...
  return (
    <div className="bg-[#FDFBF7] overflow-x-hidden selection:bg-[#2C3E50] selection:text-white font-sans text-[#2C3E50]">
      ...
      {/* =========================================
          FŐ TARTALOM (TÜRKIZ vagy ROSE FEHÉR OLDAL)
          ========================================= */}
...
{/* --- NAVBAR --- */}
      <nav className={`fixed w-full z-50 transition-all duration-700 ease-[0.22,1,0.36,1] ${isScrolled ? "py-4 bg-white border-none" : "py-6 bg-gradient-to-b from-black/80 via-black/40 to-transparent"}`}>
        <div className="relative z-10 max-w-[90rem] mx-auto px-6 lg:px-12 flex justify-center md:justify-between items-center">
            ...
            <div className="w-full bg-[#0B131A] text-white flex flex-col">
              ...
              <section className="relative z-10 py-24 lg:py-32 overflow-hidden">
                ...
                <div className="relative z-10 max-w-4xl mx-auto px-6 lg:px-12 flex flex-col items-center text-center">
                   ...
                   <h2 className="text-4xl md:text-5xl font-serif mb-6 text-white">{t.sister.title}</h2>
                   <p className="text-white/80 text-[14px] md:text-[15px] font-sans leading-relaxed max-w-2xl mb-10">{t.sister.desc}</p>
          ...
          >
             <img src="/rose-logo.png" alt="Rose" className="h-[200px] object-contain filter invert brightness-0 mb-8" />
             <h1 className="text-3xl md:text-5xl font-serif text-[#640E01] mb-4 text-center">Rose Meze & Cocktail Bar</h1>
             <p className="text-black/60 font-sans tracking-widest uppercase text-xs">Ide jön a Rose weboldala...</p>
          </motion.div>
        )}
      </AnimatePresence>
...
{/* =========================================
          ÚJ STICKY OLDAL SWITCHER (KÖZÉPEN ALUL)
          ========================================= */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999] flex items-center p-1.5 bg-[#0B131A]/90 backdrop-blur-xl rounded-full border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.5)]">
        
        {/* TÜRKIZ GOMB */}
        <button 
          onClick={() => {
            setIsLoading(true);
            setActiveSite("turkiz");
          }}
          className={`relative w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center transition-all duration-500 overflow-hidden ${activeSite === "turkiz" ? "bg-[#62B6C7] text-white" : "bg-transparent text-white/40 hover:bg-white/5 hover:text-white"}`}
        >
          <TurkizIcon className="w-8 h-8 md:w-10 md:h-10 relative z-10" />
        </button>
        ...
        <button 
          onClick={() => {
            setIsLoading(false); // Nincs preloader a rose-nál
            setActiveSite("rose");
          }}
          className={`relative w-14 h-14 md:w-16 md:h-16 rounded-full flex items-center justify-center transition-all duration-500 overflow-hidden ${activeSite === "rose" ? "bg-[#430D01]" : "bg-transparent hover:bg-white/5 opacity-50 hover:opacity-100"}`}
        >
          <img src="/rose-logo-big.png" alt="Rose" className="w-8 h-8 md:w-10 md:h-10 object-contain relative z-10" />
        </button>
